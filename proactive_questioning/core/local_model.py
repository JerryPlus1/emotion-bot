...
class LocalModel:
    ...
    def _extract_answer(self, text: str) -> str:
        ...
        # 6. 移除选项提示（如"或者："、"选择其中一个"）
        text = re.sub(r'或者[：:]\s*', '', text)
        text = re.sub(r'选择其中一个.*?(?:即可|作为回答)', '', text)
        
        # 7. 只保留第一段回复（过滤掉重复输出）
        # 方法：检测"段落"模式，只保留第一个段落
        # 段落定义：以第一个非空行为起点，到下一个非空行（如果与前面重复则停止）
        lines = text.split('\n')
        first_line = None
        result_lines = []
        hit_duplicate = False
