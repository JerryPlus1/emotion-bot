...
前端右侧可以写入知识库文本，也可以选择本地文本文件上传。选择文件后，页面会读取文件内容并填入知识库正文，点击“写入”后会进入当前 `user_id` 的知识库；之后聊天会根据“知识库”权重检索这些文件内容。

当前前端适合上传文本类文件，例如 `.txt`、`.md`、`.csv`、`.json`、`.log`。如果中文文本出现乱码，可以把文件编码切换为 `GB18030` 后重新选择文件。

服务端会自动切分文本并建立 embedding 索引。
