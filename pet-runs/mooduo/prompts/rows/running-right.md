Create a single horizontal sprite strip for the Codex app digital pet `mooduo` in the state `running-right`.
...
Output exactly 8 separate animation frames arranged left-to-right in one single row. Each frame must show the same pet: Mooduo is a compact chibi digital pet inspired by a Python emotion-bot builder: a small rounded teal-and-warm-coral companion with expressive mood-sensor eyes, a tiny chat-bubble ear shape, and subtle bilingual charm without visible text..
